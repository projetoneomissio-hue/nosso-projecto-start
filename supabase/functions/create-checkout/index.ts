import { serve } from "https://deno.land/std@0.190.0/http/server.ts";
import Stripe from "https://esm.sh/stripe@18.5.0";
import { createClient } from "https://esm.sh/@supabase/supabase-js@2.57.2";

const corsHeaders = {
  "Access-Control-Allow-Origin": "*",
  "Access-Control-Allow-Headers": "authorization, x-client-info, apikey, content-type",
};

interface CheckoutRequest {
  pagamentoId: string;
  returnUrl?: string;
}

serve(async (req) => {
  if (req.method === "OPTIONS") {
    return new Response(null, { headers: corsHeaders });
  }

  const supabaseClient = createClient(
    Deno.env.get("SUPABASE_URL") ?? "",
    Deno.env.get("SUPABASE_ANON_KEY") ?? ""
  );

  try {
    console.log("[CREATE-CHECKOUT] Function started");
    
    const authHeader = req.headers.get("Authorization");
    if (!authHeader) throw new Error("No authorization header");
    
    const token = authHeader.replace("Bearer ", "");
    const { data: userData, error: userError } = await supabaseClient.auth.getUser(token);
    if (userError || !userData.user) throw new Error("User not authenticated");
    
    const user = userData.user;
    console.log("[CREATE-CHECKOUT] User authenticated:", user.email);

    const { pagamentoId, returnUrl } = await req.json() as CheckoutRequest;
    if (!pagamentoId) throw new Error("pagamentoId is required");

    // Fetch payment details from database
    const supabaseService = createClient(
      Deno.env.get("SUPABASE_URL") ?? "",
      Deno.env.get("SUPABASE_SERVICE_ROLE_KEY") ?? ""
    );

    const { data: pagamento, error: pagError } = await supabaseService
      .from("pagamentos")
      .select(`
        id, valor, data_vencimento, status,
        matricula:matriculas!inner(
          aluno:alunos!inner(
            nome_completo,
            responsavel_id
          ),
          turma:turmas!inner(
            nome,
            atividade:atividades!inner(nome)
          )
        )
      `)
      .eq("id", pagamentoId)
      .single();

    if (pagError || !pagamento) throw new Error("Payment not found");
    
    const matricula = pagamento.matricula as any;
    const aluno = matricula.aluno;
    
    // Verify user is the responsible party
    if (aluno.responsavel_id !== user.id) {
      throw new Error("You don't have permission to pay this");
    }

    if (pagamento.status === 'pago') {
      throw new Error("Payment already completed");
    }

    console.log("[CREATE-CHECKOUT] Creating Stripe session for payment:", pagamentoId);

    const stripe = new Stripe(Deno.env.get("STRIPE_SECRET_KEY") || "", {
      apiVersion: "2025-08-27.basil",
    });

    // Check if customer exists
    const customers = await stripe.customers.list({ email: user.email || "", limit: 1 });
    let customerId;
    if (customers.data.length > 0) {
      customerId = customers.data[0].id;
    }

    const origin = req.headers.get("origin") || returnUrl || "https://localhost:3000";
    const turma = matricula.turma;
    const atividade = turma.atividade;
    
    // Convert value to cents (Stripe uses smallest currency unit)
    const amountInCents = Math.round(Number(pagamento.valor) * 100);

    const session = await stripe.checkout.sessions.create({
      customer: customerId,
      customer_email: customerId ? undefined : user.email,
      payment_method_types: ["card", "pix"],
      line_items: [
        {
          price_data: {
            currency: "brl",
            product_data: {
              name: `Mensalidade - ${atividade.nome}`,
              description: `Aluno: ${aluno.nome_completo} | Turma: ${turma.nome} | Venc: ${pagamento.data_vencimento}`,
            },
            unit_amount: amountInCents,
          },
          quantity: 1,
        },
      ],
      mode: "payment",
      success_url: `${origin}/responsavel/pagamentos?success=true&payment_id=${pagamentoId}`,
      cancel_url: `${origin}/responsavel/pagamentos?canceled=true`,
      metadata: {
        pagamento_id: pagamentoId,
        aluno_nome: aluno.nome_completo,
        atividade: atividade.nome,
      },
      payment_method_options: {
        pix: {
          expires_after_seconds: 86400, // 24 hours
        },
      },
    });

    console.log("[CREATE-CHECKOUT] Session created:", session.id);

    return new Response(JSON.stringify({ url: session.url, sessionId: session.id }), {
      headers: { ...corsHeaders, "Content-Type": "application/json" },
      status: 200,
    });
  } catch (error) {
    const errorMessage = error instanceof Error ? error.message : String(error);
    console.error("[CREATE-CHECKOUT] Error:", errorMessage);
    return new Response(JSON.stringify({ error: errorMessage }), {
      headers: { ...corsHeaders, "Content-Type": "application/json" },
      status: 500,
    });
  }
});
