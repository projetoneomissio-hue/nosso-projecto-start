  ...
  try {
    ...
    switch (event.type) {
      ...
      case "checkout.session.async_payment_succeeded": {
        ...
        if (!pagamentoId) {
          console.warn("[STRIPE-WEBHOOK] No pagamento_id in metadata, skipping session:", session.id);
          break;
        }

        console.log(`[STRIPE-WEBHOOK] Processing payment: ${pagamentoId} (Session: ${session.id})`);

        // IDEMPOTENCY CHECK: Verify if payment is already paid
        ...
        if (fetchError) {
          console.error(`[STRIPE-WEBHOOK] Error fetching payment ${pagamentoId}:`, fetchError);
          // Return 500 to trigger Stripe retry if it's a transient DB error
          throw fetchError;
        }

        if (currentPayment && currentPayment.status === 'pago') {
          console.log(`[STRIPE-WEBHOOK] IDEMPOTENCY HIT: Payment ${pagamentoId} already paid. Returning 200.`);
        ...
        // For PIX/Card, check connection status
        if (session.payment_status === "paid") {
          ...
          if (error) {
            ...
            throw error; // Return 500 to retry
          }

          console.log("[STRIPE-WEBHOOK] Payment marked as paid successfully:", pagamentoId);
          ...
          try {
            ...
            if (customerEmail) {
              ...
              const emailRes = await fetch(`${Deno.env.get("SUPABASE_URL")}/functions/v1/send-email`, {
                ...
                body: JSON.stringify({
                  to: customerEmail,
                  subject: "Pagamento Confirmado - Neo Missio",
                  html: `
              ...
              if (!emailRes.ok) {
                console.error(`[STRIPE-WEBHOOK] Email API failed: ${emailRes.status} - ${await emailRes.text()}`);
              }
            }
          } catch (emailErr) {
            console.error("[STRIPE-WEBHOOK] Failed to send email (non-critical):", emailErr);
          }
        }
        break;
      }
